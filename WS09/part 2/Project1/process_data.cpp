...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var) {
		// create vectors for threads
		std::vector<std::thread> f_threads;
		std::vector<std::thread> v_threads;

		//bind computeAvgFactor 
		for (int i = 0; i < num_threads; i++) {
		...
		// join all the average threads
		for (int i = 0; i < num_threads; i++) {
			f_threads.at(i).join();
			avg += averages[i];
		}

		//bind computeVarFactor
		for (int i = 0; i < num_threads; i++) {
			v_threads.push_back(std::thread(std::bind(&computeVarFactor, &data[p_indices[i]], p_indices[i + 1] - p_indices[i], total_items, avg, std::ref(variances[i]))));
		}

		// join all the variance threads
		for (int i = 0; i < num_threads; i++) {
			v_threads.at(i).join();
			var += variances[i];
		}
