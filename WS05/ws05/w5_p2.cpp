...
std::string err = "♪ Hey, I just met you, ♪\n"
"♪ And this is crazy, ♪\n"
...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		if (file) {  	
			size_t cnt = 0;  //inner count
			std::string bookLine;
			while (cnt < 4 && std::getline(file, bookLine)) {
			...
			std::cerr << err;
			std::exit(AppErrors::CannotOpenFile);//       - if the file cannot be open, print a message to standard error console and exit from application with error code "AppErrors::CannotOpenFile"
		}
		file.close(); 
	}
	...
	// TODO: add the rest of the books from the file.
		if (argc == 5) {
			...
			if (file) {
				size_t cnt = 0;  //inner count
				std::string bookLine;
				while (std::getline(file, bookLine) && file) {
					if (bookLine[0] != '#') {  //ines that start with "#" are considered comments and should be ignored
						sdds::Book book(bookLine);//read one line at a time, and pass it to the Book constructor
						library += book;  //store each book read into the collection "library" (use the += operator)
						cnt++;  // Increment the count
	...
	//from part #1
	// TODO: iterate over the library and fix each book
	for (auto i = 0u; i < library.size(); i++) {
		fixedPrice(library[i]);
	}
