/*************** MS 2 - CustomerOrder Module -  CustomerOrder.cpp ********************
 Name: Julia Alekseev
 Seneca Student ID: 051292134
 Seneca email: jalekseev@myseneca.ca
 Date of completion: 2023-11-12
...
namespace sdds {
	...
	{
		bool ok = false;
		for (auto i = 0u; i < m_cntItem && ok; ++i) {
			ok = m_lstItem[i]->m_isFilled == true;
			ok = true;
		}
		return ok;
	}
	...
	void CustomerOrder::fillItem(Station& station, std::ostream& os) {
		...
		for (auto i = 0u; i < m_cntItem && ok; ++i) {
			if (m_lstItem[i]->m_itemName == station.getItemName()) {
				...
				if (station.getQuantity() >= 1) { //if the order contains items handled, and the Station's inventory contains at least one item
