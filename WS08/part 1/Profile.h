...
namespace sdds {
	...
	struct Profile {
		...
		//Add a validateAddress() function to the Profile type. Your function reports an exception if the address’s number is negative or postal code does not follow ANA NAN format. Your function receives no parameters.
		void validateAddress() {
			         //if negative                            //postal code + space 7 charachetrs          //if 0 loc not upper char                          
			if (this->m_address.number < 0 || this->m_address.postal_code.length() != 7 || !isupper(this->m_address.postal_code[0]) || 
			      	//if 1 loc not digist                   //if 2 loc not upper char                    //if 3 loc not space 
				!isdigit(this->m_address.postal_code[1]) || !isupper(this->m_address.postal_code[2]) || this->m_address.postal_code[3] != ' ' ||
			     	//if 4 loc not digist                             //if 5 loc not upper char               //if 6 loc not digist 
				!isdigit(this->m_address.postal_code[4]) || !isupper(this->m_address.postal_code[5]) || !isdigit(this->m_address.postal_code[6])) 
			{
			throw std::string("*** Invalid Address ***");  //throwing if ok is false
			}
		}
